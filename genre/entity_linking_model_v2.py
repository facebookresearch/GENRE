...
class GENREForEndToEndEntityLinking(object):
    ...
    def __init__(self, genre_model, mention_trie_file, candidates_dict_file):
        ...
        self.codes = {
            k: self.bart.encode(" {}".format(k))[1].item()
            for k in ("{", "}", "[", "]")
        }
        self.codes["EOS"] = 2
    ...
    def from_pretrained(self, *args, mention_trie_file, candidates_dict_file, **kwargs):
        return GENREForEndToEndEntityLinking(GENRE.from_pretrained(*args, **kwargs), mention_trie_file, candidates_dict_file)
    ...
    def sample(self, inputs, beam=6, max_len_b=1024):
        ...
        def get_trie_outside(sent, sent_orig):
            ...
            if pointer_end:
                if sent_orig[pointer_end] != self.codes["EOS"] and sent_orig[pointer_end] in self.mention_trie.get([]):
