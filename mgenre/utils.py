...
def add_to_trie(sequence, trie):
    if sequence != []:
        if sequence[0] not in trie:
            trie[sequence[0]] = {}
        add_to_trie(sequence[1:], trie[sequence[0]])


def remove_from_trie(sequence, trie):
    if sequence == []:
        return trie == {}
    else:
        if sequence[0] not in trie:
            return False
        elif remove_from_trie(sequence[1:], trie[sequence[0]]):
            if len(trie) != 1:
                del trie[sequence[0]]
                return False
            else:
...
def get_from_trie_dict(indices, trie):
    if len(indices) == 0:
        return list(trie.keys())
    elif indices[0] not in trie:
        return []
    else:
        return get_from_trie_dict(indices[1:], trie[indices[0]])
                ...
                if status == "o":
                    ...
                    if output_[j] == "[":
                        ...
                        if len(entities[-1][2]) <= 1:
                            ...
                            redirections is not None and entities[-1][2] in redirections
                        ):
